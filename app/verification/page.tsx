...
export default function VerificationPage() {
  ...
  const isComplianceComplete = () => {
    return formData.agreeTerms && formData.agreePrivacy && formData.agreeAML && formData.agreeDataProcessing
  }
      ...
      <div className="container max-w-4xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-placebo-black mb-2">
            {language === "en" ? "Complete User Verification" : "ユーザー認証を完了"}
          </h1>
          <p className="text-gray-600">
            {language === "en"
              ? "Complete user verification to message sellers, make purchases, and build trust with other users."
              : "完全なユーザー認証を完了して、販売者にメッセージを送信したり、購入したり、他のユーザーとの信頼関係を構築したりできます。"}
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {language === "en" ? "User Verification Process" : "ユーザー認証プロセス"}
            </CardTitle>
            <CardDescription>
              {language === "en"
                ? "All users must complete user verification. Your information is encrypted and securely stored. Your submission will be reviewed by our team within 24-48 hours."
                : "すべてのユーザーはユーザー認証を完了する必要があります。あなたの情報は暗号化され、安全に保管されます。提出内容は24-48時間以内にチームが確認いたします。"}
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="mt-6">
                ...
                <TabsContent value="review">
                  <div className="space-y-6">
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-amber-700">
                          {language === "en"
                            ? "Please review all information carefully. Once submitted, your verification will be reviewed by our team within 24-48 hours. You will receive an email notification with the results."
                            : "すべての情報を慎重に確認してください。送信後、24-48時間以内にチームが認証を確認いたします。結果についてはメール通知をお送りします。"}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Review sections */}
                    <div className="space-y-4">
                      <h3 className="font-medium text-lg">{language === "en" ? "Personal Information" : "個人情報"}</h3>
                      <div className="grid grid-cols-2 gap-y-2 text-sm bg-gray-50 rounded-lg p-4">
                        <div className="text-gray-500">{language === "en" ? "Full Name" : "氏名"}</div>
                        <div>
                          {formData.firstName} {formData.lastName}
                        </div>
                        <div className="text-gray-500">{language === "en" ? "Date of Birth" : "生年月日"}</div>
                        <div>{formData.dateOfBirth}</div>
                        <div className="text-gray-500">{language === "en" ? "Email" : "メール"}</div>
                        <div>{formData.email}</div>
                        <div className="text-gray-500">{language === "en" ? "Phone" : "電話"}</div>
